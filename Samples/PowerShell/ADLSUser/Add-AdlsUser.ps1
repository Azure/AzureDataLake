<#
.SYNOPSIS
Script used to add full permissions for a Data Lake Store user

.DESCRIPTION
This script adds permissions for the specified user, service principal, or group to access 
files and directories in the specified Data Lake Store account, with the specified level of access.

.PARAMETER Account
The name of the Data Lake Store account to which to add the user.
...
function copyacls
{
	...
	foreach($item in $itemList)
	{
		...
		{
			# set the ACL without default using "rwx" permissions
			giveaccess -Account $Account -Path $Path -IdToAdd $IdToAdd -entityType $entityType -permissionToAdd "rwx" -loginProfilePath $loginProfilePath | Out-Null
		}
		elseif ($item.Type -ieq "DIRECTORY")
		{
			# set permission and recurse on the directory
			giveaccess -Account $Account -Path $Path -IdToAdd $IdToAdd -entityType $entityType -permissionToAdd "rwx" -isDefault -loginProfilePath $loginProfilePath  | Out-Null
...
{	
    ...
    copyacls -Account $Account -Path $Path -IdToAdd $EntityIdToAdd -entityType $EntityType -loginProfilePath $profilePath | Out-Null
}
catch
{
